...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=re \
		  -X github.com/cosmos/cosmos-sdk/version.AppName=tic-chaind \
...
ifeq ($(OS),Windows_NT)
	...
	go build -mod=readonly $(BUILD_FLAGS) -o build/tic-chaind ./cmd/tic-chaind
endif
...
build-static-linux-amd64: go.sum $(BUILDDIR)/
	...
	$(DOCKER) cp rebinary:/bin/tic-chaind $(BUILDDIR)/tic-chaind-linux-amd64
	$(DOCKER) rm -f rebinary

install: check_go_version
	go install -mod=readonly $(BUILD_FLAGS) ./cmd/tic-chaind

install-test-binary: check_go_version
	go install -mod=readonly $(BUILD_FLAGS_TEST_BINARY) ./cmd/tic-chaind
...
draw-deps:
	...
	@goviz -i ./cmd/tic-chaind -d 2 | dot -Tpng -o dependency-graph.png
